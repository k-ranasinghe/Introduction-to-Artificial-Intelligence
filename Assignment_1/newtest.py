...
if __name__ == "__main__":
    ...
    with open(output_file, 'w') as f:
        ...
        for node, letter in node_mapping.items():
            f.write(f"{letter} : {', '.join(str(x) if x != np.inf else 'N' for x in city_map[node])}\n")

        # Truck Names and Capacities
        f.write("\nTrucks:\n")
        for truck in trucks:
            truck_name, truck_capacity = truck.split('#')
            f.write(f"{truck_name}: Capacity {truck_capacity}\n")
            paths = traverse_map(city_map, int(truck_capacity), 0)
            shortest_path, shortest_distance = paths[0]
            f.write(f"Shortest Path for {truck_name}: {' -> '.join([node_mapping[i] for i in shortest_path])} ({shortest_distance})\n\n")
